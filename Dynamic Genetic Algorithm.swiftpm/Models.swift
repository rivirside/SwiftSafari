//I apologize for the cringe humour...
...
//What are we if not vectors in biological meat puppets?
class Vector: ObservableObject {
    ...
    func limit(_ max: Float) {
        ...
        let maxSquared = max * max // You're not a square Max, you're great!
...
class Dot: Identifiable, ObservableObject {
    ...
    init(dead: Bool, success: Bool, champion: Bool, position: Vector, neuralNetwork: NeuralNetwork, direction: Vector, acceleration: Vector, velocity: Vector, maxWidth: Int, maxHeight: Int, minTargetDistance: Float, dotSize: Int) {
        self.dead = dead
    ...
    func update(target: Vector) {
        ...
        // Check if the dot has gone out of bounds
        if outBounds() {
            self.dead = true
            return
        }
...
// Here come the reinforcements! Let's welcome the Population class!
// MARK: Population
class Population: ObservableObject {
    ...
    // The fittest survive, and they have a 50% chance
    ...
    init(numDots: Int, targetPosition: Vector, width: Int, height: Int, dotSize: Int, minTargetDistance: Float) {
        ...
        // Behold the target! This is where our dots aspire to be
    ...
    // It's time to update our dots! Let's see how they fare against the target
    func update(target: Vector) {
        dead = 0
        winners = 0
    ...
    // Time for a fresh start, let's reset the view
    func resetView() {
        ...
        // Record breaking performance? Let's find out!
    ...
    // Time for a new generation to take the stage
    ...
    // Let's put the dots in order, shall we?
    func sortDots() {
    ...
    // Find an easy target for the dots
    func getEasyTarget() {
    ...
    // Generate a random vector within the specified bounds
    ...
    // Count the number of successful dots
    ...
    // Mutate the dots to improve their performance
    func mutate() {
        print("mutate")
        for dot in dots {
    ...
    // Make the first dot the champion
    func makeFirst() {
        print("makeFirst")
    ...
    // Select the next generation of dots
    func selection() {
        print("selection")
        var index = 0
    ...
    // Another selection method for the next generation of dots
    func selection2() {
        print("selection2")
        historySort()
    ...
    // Set up the next generation of dots
    func setupNext() {
        sortDots()
    ...
    // Sort dots based on their history
    func historySort() {
        dots.sorted { $0.history > $1.history }
    }
    
    // Clear the history of each dot
    ...
    // Check and print the scores of dots
    func checkScores() {
        var scores: [Double] = []
